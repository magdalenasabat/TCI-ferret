...
function varargout=sigstar(h,groups,stats,nosort,ylimMethod)
    % sigstar - Add significance stars to bar charts, boxplots, line charts, etc,
    %
    % H = sigstar(h,groups,stats,nsort)
    %
    % Purpose
    ...
    % R(:,1)=R(:,1)+3;
    % boxplot(R)
    % set(h,'XTick',1:2,'XTickLabel',{'A','B'})
    % H=sigstar({{'A','B'}},0.01);
    % ylim([-3,6.5])
    % set(H,'color','r')
    %
    ...
    y=ylim(h);
    % i want the bars at a fixed position from the top of the plot
    switch ylimMethod
        case 'data'
            yd=myRange(y)*0.1; %separate sig bars vertically by 10% 
            for ii=1:length(groups)
                thisY= findMinY(h,xlocs(ii,:)) + yd;
                H(ii,:)=makeSignificanceBar(h,xlocs(ii,:),thisY,stats(ii));
            end 
        case 'ylim'
            thiLim = ylim;
            yd=thiLim(2)*0.2; %separate sig bars vertically by 10% 
            for ii=1:length(groups)
                thisY= thiLim(2) + yd;
                H(ii,:)=makeSignificanceBar(h,xlocs(ii,:),thisY,stats(ii));
                thiLim(2)=thiLim(2)+yd;
            end 
                
            
        otherwise
            error('no y-axis aligement chosen')
    end
    ...
    %for all bars.
    yd=myRange(ylim(h))*0.02; %Ticks are 2% of the y axis range
    for ii=1:length(groups)
        y=get(H(ii,1),'YData');
        y(1)=y(1)-yd;
        y(4)=y(4)-yd;   
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%Internal functions
function H=makeSignificanceBar(h,x,y,p)
    ...
    x=repmat(x,2,1);
    y=repmat(y,4,1);
    H(1)=plot(h,x(:),y,'-k','LineWidth',.5,'Tag','sigstar_bar');
    ...
    starY=mean(y)+myRange(ylim(h))*offset;
    H(2)=text(h,mean(x(:)),starY,stars,...
        'HorizontalAlignment','Center',...
        'BackGroundColor','none',...
        'Tag','sigstar_stars');
    Y=ylim(h);
    if Y(2)<starY
        ylim(h,[Y(1),starY+myRange(Y)*0.05])
    end
end %close makeSignificanceBar
function Y=findMinY(h,x)
    ...
    % fails anyway
    if ~isa(get(h,'XAxis'),'matlab.graphics.axis.decorator.CategoricalRuler')
        set(h,'xlim',x) %Matlab automatically re-tightens y-axis
    end
    yLim = get(h,'YLim'); %Now have max y value of all elements within range.
    Y = max(yLim);
    axis(h,'normal')
    set(h,'XLim',oldXLim,'YLim',oldYLim)
end %close findMinY
