...
if I.nperms > 0
    ...
    fprintf('permutations: 1'); drawnow;
    for z = 1:I.nperms
        ...
        for j = 2:n_reps
            if I.chunksize == 0
                if isempty(used_orders)
                    used_orders = (1:n_stimuli)';
                end
                
                if ~isnan(I.maxoverlap)
                    order_found = false;
                    while ~order_found
                        perm_order = randperm(n_stimuli)';
                        frac_overlap = mean(perm_order == used_orders);
                        order_found = all(frac_overlap < I.maxoverlap);
                    end
                else
                    perm_order = randperm(n_stimuli)';
                end
                D_perm(:,j,:) = D(:,j,perm_order);
                used_orders = cat(2, used_orders, perm_order);
            else
...
if splithalf
    ...
    r = fastcorr(mean(X_not_outliers(:,odd_runs),2), mean(X_not_outliers(:,even_runs),2));
    clear odd_runs even_runs;
else
    runs_per_split = 1;
    r_pairs = corr(X_not_outliers); % correlation of response timecourse for all pairs of runs
    r = mean(r_pairs(~eye(n_reps)),'omitnan'); % average off diagonal entries
    clear r_pairs;
end
