...
for w=1:length(widths)
    ...
    %     box off
        ...
        figure; 
        plot(outer_tsec,overlap_surround); hold on; 
        plot(outer_tsec,overlap_surround2); 
        plot(outer_tsec,overlap_central)
        legend('overlap surround diff','overlap surround same', 'overlap central')
        xlabel('time')
        box off
...
%     'lossfn', 'sqerr', 'nintper', 20, 'boundstrength', [1]);
    ...
    M{w} = modelfit_cross_context_corr(L, 'overwrite', true, 'plot_figure', true, ...
        'shape', [1,2,3,4,5], 'intper_range', [1/128, 0.25], 'delay_range', [0, 0.25], ...
        'lossfn', 'sqerr', 'nintper', 50, 'boundstrength', [0, 0.25, 0.5, 1, 2],'intervalmass', 0.75);
end
...
function [onsets, durations] = generate_trial_with_durations(central_segment_dur,duration_segments, time_range, sampling_rate)
    ...
    onsets = [-fliplr([onsets_before(2:end) sum(durations_before)]),  onsets_after+central_segment_dur];
    durations = [fliplr(durations_before), durations_after];
end
