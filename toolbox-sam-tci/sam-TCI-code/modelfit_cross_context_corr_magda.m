...
% added by Magda
if isfield(L, 'surrounding_segments')
    I.surrounding_segments = L.surrounding_segments;
    I.n_total_segs = L.n_total_segs*2;
else
    error('Not implemented')
%     I.surrounding_segments = [repmat(NaN);segdur_sec];
%     I.n_total_seg = [1];
end

% added by magda - precompute winpoverlap

% for o=1:length(L.unique_segs)
%     precumputed_windows_h
%     
% end
        ...
        for m = 1:length(M.shape)
            for i = 1:length(M.intper_sec)
                ...
                for k = 1:n_valid_segdurs
                    winpow=zeros(size(I.surrounding_segments{k},2),size(L.lag_t,2));
                    overlap=zeros(size(I.surrounding_segments{k},2),size(L.lag_t,2));
              
                    tic
                    fprintf('Progress: 1 %%')
                    for tr=1:size(I.surrounding_segments{k},2)
                        
                        if (mod(round(tr/size(I.surrounding_segments{k},2)*100),10)==0)
                            fprintf('%d %%', floor(tr/size(I.surrounding_segments{k},2)*100))
                        end
                        
                        [winpow(tr,:), ~, overlap(tr,:)] = win_power_ratio_magda(L.unique_segs(valid_seg_durs(k))/1000, ...
                        ...
                        'surrounding_segments',cell2mat(I.surrounding_segments{k}(:,tr)));
                    end
                    fprintf('\n')
                    toc
                    
                    if I.winpowratio
                        predictor_notrans = mean(winpow,1);
                    else
                        predictor_notrans = mean(overlap,1);
                    end
