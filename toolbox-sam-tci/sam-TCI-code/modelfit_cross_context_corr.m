...
%% Magda fix- the fitting will fail if there's only one channel, double it

flag_1chann=0;
if length(L.channels)==1
    flag_1chann=1;
    L.channels = [L.channels,L.channels+1];
    L.chnames = {L.chnames{:},num2str(L.channels(2))};
    L.diff_context = repmat(L.diff_context,1,1,2);
    L.same_context = repmat(L.same_context,1,1,2);
    L.same_context_err = repmat(L.same_context_err,1,1,2);
    L.splits_diff_context = repmat(L.splits_diff_context,1,1,2);
    L.splits_same_context = repmat(L.splits_same_context,1,1,2);
    L.splits_same_context_err = repmat(L.splits_same_context_err,1,1,2);
...
% can optionally and just return this file % edited by Magdalena
% MAT_file = mkpdir([I.output_directory '/model_fit_' L.indiv '_' param_string_modelfit '.mat']);
MAT_file = mkpdir(strcat([I.output_directory filesep 'model_fit_' char(param_string_modelfit) '.mat']));
...
if ~exist(MAT_file, 'file') || I.overwrite
    ...
    valid_seg_durs = find(M.n_total_segs(:,1)>10)'; %change to 10 magdalena
   ...
   %%
    if flag_1chann
        ...
        dfields = {
            ...
            'same_context', ...
            'same_context_err', ...
            'diff_context_bestpred', ...
            'splits_best_intper_sec', ...
            'splits_best_delay_sec_start', ...
            'splits_best_delay_sec_median', ...
            'splits_best_shape', ...
            'splits_best_boundstrength', ...
            };
        ...
        dfields = {
            'best_loss', ...
            'best_intper_sec', ...
            'best_delay_sec_start', ...
            'best_delay_sec_median', ...
            'best_shape', ...
            'best_boundstrength', ...
            'av_cv_loss', ...
            'channels'
         ...
         dfields = {      
            'splits_diff_context', ...
            'splits_same_context', ...
            'splits_same_context_err', ...
            'splits_diff_context_bestpred', ...
         ...
         if isfield(M,'cv_loss')
             M.cv_loss=M.cv_loss(1,:,:);
         end
         if isfield(M,'splits_loss')
            M.splits_loss=M.splits_loss(:,:,:,:,1,:,:);
            M.splits_best_loss=M.splits_best_loss(1,:,:);
         end
...
if I.plot_figure
    ...
    for q = 1:n_channels
        ...
        fname = mkpdir([L.figure_directory '/model-fit-' L.indiv '_' param_string_modelfit '/' chname]);
        ...
        if I.nullsmps>0 && I.plot_nullsmps
            for l = 1:I.nullsmps
                fname = mkpdir([L.figure_directory '/model-fit-' L.indiv '_' param_string_modelfit '/' chname '-nullsmp' num2str(l)]);
                if ~I.skipnullpreds
        ...
        if I.plot_splits
            ...
            for s = 1:n_splits
                for p = 1:n_partitions
                    ...
                    fname = mkpdir([L.figure_directory '/model-fit-' L.indiv '_' param_string_modelfit '/' ...
                    ...
                    plot_modelfit(M.splits_diff_context(:,:,q,p,s), M.splits_same_context(:,:,q,p,s), cv_diff_context_bestpred, ...
                        M.splits_loss(:,:,M.splits_best_shape(q,p,s)==M.shape,M.splits_best_boundstrength(q,p,s)==M.boundstrength,q,p,s), M.splits_best_intper_sec(q,p,s), ...
